<?php

namespace Inventas\AppStoreConnectKit\Api\Requests\AppCustomProductPages;

use Inventas\AppStoreConnectKit\Api\Dto\AppCustomProductPageCreateRequest;
use Inventas\AppStoreConnectKit\Support\PayloadSerializer;
use Saloon\Contracts\Body\HasBody;
use Saloon\Enums\Method;
use Saloon\Http\Request;
use Saloon\Traits\Body\HasJsonBody;

/**
 * appCustomProductPages_createInstance
 */
class AppCustomProductPagesCreateInstance extends Request implements HasBody
{
    use HasJsonBody;

    protected Method $method = Method::POST;

    public function resolveEndpoint(): string
    {
        return '/v1/appCustomProductPages';
    }

    public function __construct(
        protected AppCustomProductPageCreateRequest $payload,
    ) {}

    /**
     * @return array<string, mixed>
     */
    public function defaultBody(): array
    {
        return PayloadSerializer::serialize($this->payload);
    }
}
